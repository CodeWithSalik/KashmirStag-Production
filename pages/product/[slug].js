...
const Post = ({ buyNow, addToCart, product, variants }) => {
    ...
    const checkPincode = async () => {
        let pins = await fetch(`${process.env.NEXT_PUBLIC_LHOST}/api/pincode`)
    ...
    const refreshVariant = (newsize, newcolor) => {
        let url = `${process.env.NEXT_PUBLIC_LHOST}/product/${variants[newcolor][newsize]['slug']}`
        window.location = url;
        ...
        <section className="text-gray-600 body-font overflow-hidden">
            <div className="container px-5 py-16 mx-auto">
                <div className="lg:w-4/5 mx-auto flex flex-wrap">
                    <Image alt="ecommerce" className="lg:w-1/2 w-full lg:h-auto px-24 object-cover object-top rounded" src={product.img} width={300} height={400} />
                    <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                        <h2 className="text-sm title-font text-gray-500 tracking-widest">KASHMIR STAG</h2>
                        ...
                        <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
                            <div className="flex">
                                ...
                                {Object.keys(variants).includes('grey') && Object.keys(variants['grey']).includes(size) && <button onClick={() => { refreshVariant(size, 'grey') }} className={`border-2 ml-1 bg-gray-500 rounded-full w-6 h-6 focus:outline-none ${color === 'grey' ? 'border-black' : 'border-gray-300'}`}></button>}
                                {Object.keys(variants).includes('skyblue') && Object.keys(variants['skyblue']).includes(size) && <button onClick={() => { refreshVariant(size, 'skyblue') }} className={`border-2 ml-1 bg-sky-500 rounded-full w-6 h-6 focus:outline-none ${color === 'skyblue' ? 'border-black' : 'border-gray-300'}`}></button>}
                            ...
                            <div className="flex ml-6 items-center">
                                ...
                                <div className="relative">
                                    <select value={size} onChange={(e) => { refreshVariant(e.target.value, color) }} className="rounded border appearance-none border-gray-300 py-2 focus:outline-none focus:ring-2 focus:ring-yellow-200 focus:border-yellow-500 text-base pl-3 pr-10">
                        ...
                        <div className="flex">
                            ...
                            <button onClick={() => { addToCart(slug, 1, product.price, product.title, size, color) }} className="flex ml-4 text-white bg-yellow-500 border-0 py-2 px-2 md:px-6 focus:outline-none hover:bg-yellow-600 rounded">Add to cart</button>
                            <button onClick={() => { buyNow(slug, 1, product.price, product.title, size, color) }} className="flex ml-4 text-white bg-yellow-500 border-0 py-2 px-2 md:px-6 focus:outline-none hover:bg-yellow-600 rounded">Buy Now</button>
                        ...
                        <div className="pin">
                            <input onChange={onChangePin} className='mr-4 px-2 border-4 border-yellow-300 rounded' placeholder='enter your pincode' type="text" />
                            <button onClick={checkPincode} className='mt-4 text-white bg-yellow-500 border-0 py-2 px-6 focus:outline-none hover:bg-yellow-600 rounded-md'>Check</button>
                        </div>
